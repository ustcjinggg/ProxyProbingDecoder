...
def draw_bbox(image, iou, gt_box, pred_box, gt_score=0, is_top1=False):
...
def evaluate_dino_loc(input, cls_label, bbox_label, logits, att, cams_ori, cam_total, image_names, promp_names, cfg, feat=None, TOP_K= 1):
    
    #cams = cams_ori*cfg.BASIC.GAMMA + cam_total*(1-cfg.BASIC.GAMMA)
    cams = cams_ori
    #pdb.set_trace()
    iou_all = []
    loc_gt_known = []
    ...
    image = tensor2image(input.clone().detach().cpu(), cfg.DATA.IMAGE_MEAN, cfg.DATA.IMAGE_STD)
    pd_bbox = []
    best_thre = np.zeros(batch)
    for b in range(batch):
        gt_bbox = bbox_label[b].strip().split(' ')
        gt_bbox = list(map(float, gt_bbox))
        top_bboxes, top_mask=get_topk_boxes(top5_idx[b], cams[b], cfg.DATA.LOC_SIZE, threshold=cfg.MODEL.CAM_THR)
        topk_cls, topk_loc, wrong_details=cls_loc_err(top_bboxes, cls_label[b], gt_bbox, topk=(1,5))
        cls_top1_b, cls_top5_b = topk_cls
        loc_top1_b, loc_top5_b = topk_loc
        cls_top1.append(cls_top1_b)
        ...
        #find gt_know
        cam_b = cams[b,-1, :, :]
        cam_b = cam_b.detach().cpu().numpy()

        # Resize and Normalize CAM
        cam_b = resize_cam(cam_b, size=(cfg.DATA.LOC_SIZE, cfg.DATA.LOC_SIZE))
        max_iou = 0
        final_box = [0,0,479,479]
        # Estimate BBOX
        if cfg.BASIC.ADAPT_THRE:
            for thr_id in range(0,4):
                thr = thr_id*0.1
                estimated_bbox = get_bboxes(cam_b, cam_thr=thr)
                gt_box_cnt = len(gt_bbox) // 4
                for i in range(gt_box_cnt):
                    gt_box = gt_bbox[i * 4:(i + 1) * 4]
                    iou_i = cal_iou(estimated_bbox, gt_box)
                    if iou_i > max_iou:
                        max_iou = iou_i
                        best_thre[b] = thr
                        final_box = estimated_bbox
            estimated_bbox = final_box
        else:
            estimated_bbox = get_bboxes(cam_b, cam_thr=cfg.MODEL.CAM_THR)
            #pdb.set_trace()
            # Calculate IoU
        ...
        blend, heatmap = blend_cam(image[b], cam_b, estimated_bbox)
        # Get boxed image
        boxed_image = draw_bbox(blend, iou, np.array(gt_bbox).reshape(-1,4).astype(np.int), estimated_bbox, iou, False)
        if cfg.BASIC.ADAPT_THRE:
            text='thre:%.2f' % (best_thre[b])
            pos=(280, 30)
            cv2.putText(boxed_image, text, pos, cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        # save result
    ...
    return loc_gt_known, iou_all, pd_bbox, cls_top1, cls_top5, loc_top1, loc_top5, \
            top1_loc_right, top1_loc_cls, top1_loc_mins, top1_loc_part, top1_loc_more, top1_loc_wrong, best_thre

def get_topk_boxes(cls_inds, cam_map, crop_size, topk=(1, 5), threshold=0.2):
    maxk_boxes = []
    maxk_maps = []
    for cls,label in enumerate(cls_inds):
        ...
        maxk_boxes.append([label] + estimated_bbox)
